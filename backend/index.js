...
mongoose.connect(
  "mongodb+srv://sarthakpatil7900:7900Sarthak@cluster0.z8nxplw.mongodb.net/e-commerce"
);
...
app.post("/signup", async (req, res) => {
  ...
  if (check) {
    return res.status(400).json({
      success: false,
      erros: "Existing User Found with same email id",
    });
  }
  let cart = {};
  for (let i = 0; i < 300; i++) {
...
//creating endpoint for pouplar for women section
...
const fetchUser = async (req, res, next) => {
  ...
  if (!token) {
    ...
    try {
      ...
      response
        .status(401)
        .send({ errors: "please authencticate using a valid token" });
    }
  }
};
